...
# --- Controls ---
confidence_slider = st.slider("Confidence level for range (e.g. 0.9999)", min_value=0.95, max_value=0.99999, value=0.9999, step=1e-5, format="%.5f")
dist = st.selectbox("Distribution", ["Uniform(0,1)", "Normal(0,1)", "Exponential(λ=1)"])
mode = st.radio("Plot mode", ["Sum", "Mean"], index=0)
n_samples = st.slider("Number of samples", 10, 200, 100, step=10)
frame_delay = st.slider("Animation speed (ms per frame)", 10, 2000, 1500, step=100)

# --- Distribution Setup ---
rng = np.random.default_rng(seed=42)
max_draws = 500 * 6  # max 500 samples, each with X and 5 Xᵢ
...
# --- Histogram bins ---
bin_width = 0.2
num_bins = int((x_range[1] - x_range[0]) / bin_width)
